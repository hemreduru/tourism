...
@section('content_header')
    <h1>{{ __('permissions.edit_permission') }}</h1>
@stop
                ...
                <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="{{ __('permissions.name') }}" value="{{ old('name', $permission->name) }}" required>
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('display_name') is-invalid @enderror" id="display_name" name="display_name" placeholder="{{ __('permissions.display_name') }}" value="{{ old('display_name', $permission->display_name) }}" required>
                            @error('display_name')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" placeholder="{{ __('permissions.description') }}">{{ old('description', $permission->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
