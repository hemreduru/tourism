                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="{{ __('users.name') }}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" class="form-control @error('email') is-invalid @enderror"
                                id="email" name="email" value="{{ old('email', $user->email) }}" placeholder="{{ __('users.email') }}">
                            @error('email')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label for="password">{{ __('users.password') }} <small class="text-muted">({{ __('users.leave_blank') }})</small></label>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 @error('roles') is-invalid @enderror"
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ in_array($role->id, $userRoles) ? 'selected' : '' }} data-color="{{ $role->color }}">
                                        {{ $role->display_name }}
                                    </option>
                                @endforeach
                            </select>
